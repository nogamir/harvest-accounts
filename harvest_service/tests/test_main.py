...
def test_harvest_all_accounts(
    ...
    mock_accounts_collection.find.return_value = [account]

    # Setup mocked boto3 session
    mock_session = MagicMock()
    mock_create_session.return_value = mock_session

    # Setup harvest_buckets and harvest_roles returns
    mock_harvest_buckets.return_value = [{"id": "bucket1", "name": "test-bucket"}]
    mock_harvest_roles.return_value = [{"id": "role1", "name": "test-role"}]

    # Setup mock DB collections
    mock_harvest_db.__getitem__.side_effect = lambda name: MagicMock()

    # Call the function
    main.harvest_all_accounts()

    # Assertions
    mock_accounts_collection.find.assert_called_once()
    mock_harvest_db.__getitem__.assert_any_call("buckets")
    mock_harvest_db.__getitem__.assert_any_call("roles")
